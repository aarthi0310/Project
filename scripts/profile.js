    ...
    try {
        ...
        if (!response.ok) {
            if (response.status === 403) {
                throw new Error('Access denied. Please ensure the server is configured correctly.');
            } else if (response.status === 400) {
                throw new Error('No profile data found for this phone number.');
            }
        ...
        alert(error.message || 'Failed to load profile data. Please try again.');
        return null;
    }
}
...
function loadProfileData() {
    ...
    fetchProfileData(phoneNumber).then(data => {
        if (data) {
            ...
            profilePhone.textContent = `+91${data.phoneNumber || 'Not set'}`;
            nameInput.value = data.name || '';
            emailInput.value = data.email || '';
            phoneInput.value = `+91${data.phoneNumber || ''}`;
